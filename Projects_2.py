    ...
    if file and allowed_file(file.filename):
        ...
        for f in os.listdir(os.path.join(app.config['UPLOAD_FOLDER'], filename[:-4])):
            if f[-4:] == '.zip':
                ...
                ws = wb.create_sheet("PLOT", 0)
                for x in range(0,frame.shape[0]):
                    #print(frame['BIN        '][x])
                    #data_bin[frame['X          '][x]][frame['Y          '][x]] = frame['BIN        '][x]
                    col = int(frame['X          '][x])
                    row = int(frame['Y          '][x])
                    ws.cell(row,col).value = frame['BIN        '][x]
                    if ws.cell(row,col).value == 81:
            ...
            if f[-4:] == '.DAT'or f[-4:] == '.dat':
                ...
                newdata['X'] = newdata['X'] + 3
                newdata['Y'] = newdata['Y'] + 2
                ...
                for x in range(0,newdata.shape[0]):
                    ...
                    col = int(newdata['Y'][x])
                    row = int(newdata['X'][x])
                    ws.cell(row,col).value = newdata['BIN'][x]
                    if ws.cell(row,col).value == 1:
                        ws.cell(row,col).fill = openpyxl.styles.PatternFill('solid', fgColor='80FFFF')
                    else:
                        ws.cell(row,col).fill = openpyxl.styles.PatternFill('solid', fgColor='FFA500')
                for col in range(1,ws.max_column+1):
...
def getListFiles(path):
    ...
    for files in allFileList:
        ret[files] = time.strftime("%Y/%m/%d %I:%M:%S %p",time.localtime(os.path.getmtime('C:\\Users\\minghsuantu\\Desktop\\flask try\\static\\upload\\' + files)))
    return ret
